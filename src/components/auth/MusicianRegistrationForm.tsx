...
export function MusicianRegistrationForm() {
  ...
  if (emailSent) return <div className="rounded-3xl border bg-card p-6 text-center"><span className="mx-auto grid size-14 place-items-center rounded-2xl bg-emerald-500/10 text-emerald-600"><MailCheck className="size-7" /></span><h3 className="mt-4 text-xl font-black">Revisa tu correo</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">Enviamos un enlace de confirmación a <strong>{emailSent}</strong>. Después de confirmarlo, tu solicitud quedará pendiente de aprobación.</p><Button type="button" variant="outline" className="mt-5 rounded-xl" onClick={() => setEmailSent("")}>Registrar otro correo</Button></div>;

  return <form onSubmit={submit} className="space-y-6">
    <div className="rounded-2xl bg-primary/5 p-4"><div className="flex gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><UserRoundPlus className="size-5" /></span><div><p className="font-black">Solicitud de acceso para músicos</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Crearemos tu ficha musical y la dirección revisará la solicitud antes de habilitar el portal.</p></div></div></div>
