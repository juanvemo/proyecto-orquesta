...
import { ChevronRight, Filter, ListFilter, Plus, Search, SlidersHorizontal, UserCheck, UserRoundPlus, UsersRound } from "lucide-react";
...
export default function Musicians() {
  ...
  return <div className="space-y-6 animate-in fade-in duration-300"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Operación musical</p><h1 className="mt-2 text-3xl font-black tracking-tight">Músicos</h1><p className="mt-2 text-sm text-muted-foreground">Directorio artístico, roles, instrumentos, tarifas y disponibilidad.</p></div>{canManage && <Button onClick={() => setFormOpen(true)} className="h-11 rounded-xl px-5 font-bold"><UserRoundPlus className="mr-2 size-4" /> Nuevo músico</Button>}</div>
    ...
    {membership && <MusicianFormDialog open={formOpen} onOpenChange={setFormOpen} organizationId={membership.organizationId} instruments={instruments} roles={roles} onSaved={(id) => { void load(); navigate(`/musicos/${id}`); }} />}
  </div>;
}
