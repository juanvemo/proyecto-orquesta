...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("organization_settings").select("enabled_notifications").eq("organization_id", membership.organizationId).single(),
  ...
  const save = async (event: FormEvent) => {
    ...
    const { error: settingsError } = await supabase.from("organization_settings").update({ enabled_notifications: notifications, updated_at: new Date().toISOString() }).eq("organization_id", membership.organizationId);
    setSaving(false);
  ...
  return (
    <form onSubmit={save} className="space-y-6 animate-in fade-in duration-300">
      ...
      <Tabs defaultValue="organization" className="space-y-5"><TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-2xl bg-muted/60 p-1.5 sm:w-fit"><TabsTrigger value="organization" className="rounded-xl px-4 py-2.5"><Building2 className="mr-2 size-4" /> Organización</TabsTrigger><TabsTrigger value="brand" className="rounded-xl px-4 py-2.5"><Palette className="mr-2 size-4" /> Identidad</TabsTrigger><TabsTrigger value="notifications" className="rounded-xl px-4 py-2.5"><BellRing className="mr-2 size-4" /> Notificaciones</TabsTrigger></TabsList>
        ...
        <TabsContent value="notifications"><Card className="rounded-[2rem] shadow-none"><CardContent className="p-5 sm:p-7"><div className="mb-6"><h2 className="text-lg font-black">Canales de notificación</h2><p className="text-sm text-muted-foreground">Define los canales habilitados. Las automatizaciones se conectarán en fases posteriores.</p></div><div className="max-w-2xl divide-y">{[{ key: "email" as const, label: "Correo electrónico", note: "Avisos administrativos y confirmaciones" },{ key: "push" as const, label: "Notificaciones push", note: "Preparado para la futura PWA y aplicaciones" },{ key: "whatsapp" as const, label: "WhatsApp", note: "Generación de mensajes, sin envío automático" }].map(item => <div key={item.key} className="flex items-center justify-between gap-4 py-5"><div><p className="font-bold">{item.label}</p><p className="text-sm text-muted-foreground">{item.note}</p></div><Switch checked={notifications[item.key]} onCheckedChange={checked => setNotifications(current => ({ ...current, [item.key]: checked }))} /></div>)}</div><div className="mt-6 flex items-start gap-3 rounded-2xl bg-emerald-500/10 p-4 text-emerald-800 dark:text-emerald-300"><ShieldCheck className="mt-0.5 size-5 shrink-0" /><p className="text-sm">WhatsApp no enviará mensajes automáticamente durante esta fase. Solo se prepara la preferencia de integración.</p></div></CardContent></Card></TabsContent>
      </Tabs>
    </form>
  );
}
