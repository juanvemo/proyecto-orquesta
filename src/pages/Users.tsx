...
import { CheckCircle2, Clock3, MoreHorizontal, Search, ShieldCheck, UserCheck, UserRoundPen, UserRoundX, UsersRound } from "lucide-react";
...
type Member = { id: string; user_id: string; role_id: string; approval_status: string; created_at: string; profile?: Profile; role?: Role };
...
export default function Users() {
  ...
  const load = useCallback(async () => {
    ...
    const profilesResult = ids.length ? await supabase.from("profiles").select("id,first_name,last_name,email,avatar_url").in("id", ids) : { data: [], error: null };
    const profileMap = new Map((profilesResult.data ?? []).map((profile) => [profile.id, profile]));
    const roleMap = new Map((rolesResult.data ?? []).map((role) => [role.id, role]));
    setRoles(rolesResult.data ?? []);
    setMembers(rawMembers.map((item) => ({ ...item, profile: profileMap.get(item.user_id), role: roleMap.get(item.role_id) })));
    setLoading(false);
  }, [membership]);
